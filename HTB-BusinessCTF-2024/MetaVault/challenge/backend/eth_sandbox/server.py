...
RPC_PORT = int(os.getenv("RPC_PORT", "1337"))
...
def deploy(web3: Web3, deployer_address: str) -> str:
    rcpt = sendTransaction(
        web3, {
            ...
            "data":
            Path("/home/ctf/backend/contracts/compiled/Setup.bin").read_text(),
        })
...
def getChallengeAddress(web3: Web3, address):
    abi = json.loads(Path("/home/ctf/backend/contracts/compiled/Setup_meta.json").read_text())["output"]["abi"]
...
if __name__ == "__main__":
    launch_node()
    app.run(host="0.0.0.0", port=RPC_PORT, threaded=False)
